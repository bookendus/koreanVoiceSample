# code base on https://raw.githubusercontent.com/GSByeon/multi-speaker-tacotron-tensorflow/master/recognition/alignment.py
...
from utils import load_json, write_json, parallel_run, remove_postfix, backup_file, write_csv
...
def align_text_fn(
    ...
    # Normalization
    recognition_text_noramlized = normalize([recognition_text])
    if debug:
        print("normalized ==>", recognition_text_noramlized)
    result = { audio_name: [recognition_text, recognition_text_noramlized[0]] }

    return result

def align_text_batch(config):
    align_text = partial(align_text_fn,
            score_threshold=config.score_threshold, debug=False)
...
if __name__ == '__main__':
    ...
    write_csv(metadata_path, results)
